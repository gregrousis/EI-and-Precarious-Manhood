...
##outliers for PM close, PM Self, False Consensus, and loneliness
outlier_dataPMClose <- clean_data %>% 
  select(c("PMSelf", "PMClose", "EI", "FalseCons", "lonely"))

outlier_dataPMClose <- outlier_dataPMClose %>% 
  filter(!(is.na(PMSelf))) %>% 
...
PM_out <- aq.plot(outlier_dataPMClose)

out_data <- cbind(outlier_dataPMClose, PM_out$outliers)

out_dataPMClose <- out_data %>% 
  filter(PM_out$outliers == FALSE)

chisq.plot(outlier_dataPMClose, quan = 0.5, ask = TRUE)

### outliers from chi-square plot:  40 41 70 77 76 174 19 31 222 88 230 200 3 117 26 151 184 86 36 18 98 199 181 96 127 148 166 189 142 135 195 59 155 73

data_PmClose <- clean_data %>% 
  filter(!ID %in% c(40L, 41L, 70L, 77L, 76L, 174L, 19L, 31L, 222L, 88L, 230L, 200L, 3L, 117L, 26L, 151L, 184L, 86L, 36L, 18L, 98L, 199L, 181L, 96L, 127L, 148L, 166L, 189L, 142L, 135L, 195L, 59L, 155L, 73L))
...
mod1 <- lm(EI ~ FalseCons + PMSelf + PMClose + lonely + PMSelf*PMClose,
           data = out_dataPMClose)
